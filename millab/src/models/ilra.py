...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, Q, K, return_attention=False):
        ...
        A, attention_weights = self.multihead_attention(
            Q, K, V, need_weights=return_attention, average_attn_weights=True
        )  # A is shaped S_Q, B, D_V
        attention_weights = (
            attention_weights.transpose(0, 1) if attention_weights is not None else None
        )
...
class GAB(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, num_heads, num_inds, ln=False):
        ...
        self.latent = nn.Parameter(
            torch.Tensor(1, num_inds, dim_out)
        )  # low-rank matrix L

        nn.init.xavier_uniform_(self.latent)

        self.project_forward = MultiHeadAttention(
            dim_out, dim_in, dim_out, num_heads, ln=ln, gated=True
        )
        self.project_backward = MultiHeadAttention(
    ...
    def forward(self, X):
        ...
        H, _ = self.project_forward(
            latent_mat, X
        )  # project the high-dimensional X into low-dimensional H
        X_hat, _ = self.project_backward(
            X, H
        )  # recover to high-dimensional space X_hat
...
class NLP(nn.Module):
    ...
    def forward(self, X, return_attention=False):
        ...
        ret, attention = self.mha(
            global_embedding, X, return_attention=return_attention
...
class ILRA(MIL):
    def __init__(
        ...
        for idx in range(num_attention_layers):
            block = GAB(
                dim_in=first_dim if idx == 0 else embed_dim,
                dim_out=embed_dim,
                num_heads=num_heads,
                num_inds=topk,
                ln=ln,
    ...
    def forward_features(self, x, return_attention=False):
        ...
        for block in self.gab_blocks:
            x = block(x)
        slide_feat, attention = self.forward_attention(
    ...
    def forward(
        ...
        return_moe_weights=False,
        **kwargs,
    ):
        if self.mode == "classification":
            slide_feats, attention = self.forward_features(
            ...
            log_dict = {
                "loss": cls_loss.item() if cls_loss is not None else -1,
            }
            if return_attention:
