...
class CLAMSB(MIL):
    ...
    def __init__(
        ...
        # Instance Embedding MLP or Mammoth MoE when moe_args specifies num_experts
        ...
        self.global_attn = attn_func(
            L=embed_dim, D=attention_dim, dropout=dropout, num_classes=1
        ...
        if instance_loss_fn == "svm":
            ...
            try:
                ...
                raise AssertionError(
                    "SmoothTop1SVM requires CUDA to be available. If not available, please use the ce loss function"
                )
        else:
    ...
    def forward_attention(
        self, h: torch.Tensor, attention_only: bool = False
        ...
        """
        # Apply instance embedding (Mammoth expects [B, M, D]; create_mlp path expects [M, D])
        if isinstance(self.patch_embed, Mammoth):
        ...
        A = torch.transpose(
            A, 1, 0
        )  # Transpose to [1 x M] for consistency with CLAM's original logic
        ...
        if attention_only:
            ...
            return (
                h,
                A,
            )  # h (embedded instances) [M, E], A (unnormalized attention) [1, M]
    ...
    def forward_features(
        self, h: torch.Tensor, return_attention: bool = True
        ...
        h_embedded, attention = self.forward_attention(
            h
        )  # h_embedded: [M, E], attention: [1, M]
    ...
    def forward_instance_heads(
        ...
        inst_labels = (
            F.one_hot(label, num_classes=self.num_classes).squeeze(0).to(label.device)
        )

        for i in range(len(self.instance_classifiers)):
            inst_label_for_class = inst_labels[
                i
            ].item()  # Get 0 or 1 for current class 'i'
            ...
            if inst_label_for_class == 1:  # If the bag belongs to class 'i'
                ...
                if (
                    self.subtyping
                ):  # Only evaluate out-of-class instances for subtyping problems
                    instance_loss, _, _ = self.inst_eval_out(
                        attention_scores, h, classifier
                    )
                    total_inst_loss += instance_loss
        ...
        if self.subtyping and len(self.instance_classifiers) > 0:
            ...
            not self.subtyping and inst_labels.sum().item() > 0
        ):  # If there was at least one positive class
            total_inst_loss /= (
                inst_labels.sum().item()
            )  # Average over positive classes only
        elif (
            total_inst_loss == 0 and inst_labels.sum().item() == 0
        ):  # No positive classes, and not subtyping
    ...
    def forward(
        ...
        slide_feats, intermeds = self.forward_features(
            h, return_attention=return_attention
        )
        ...
        log_dict = {
            ...
            "cls_loss": loss_fn(logits, label).item()
...
class CLAMConfig(PretrainedConfig):
    ...
    Args:
        ...
        moe_args (dict): Optional Mammoth MoE args; if num_experts > 0, patch_embed uses Mammoth (default: {}).
    """
