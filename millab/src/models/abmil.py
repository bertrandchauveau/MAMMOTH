...
class ABMIL(MIL):
    ...
    def __init__(
        ...
        self.global_attn = attn_func(
            L=embed_dim, D=attn_dim, dropout=dropout, num_classes=1
        )
    ...
    def forward_features(
        self, h: torch.Tensor, attn_mask=None, return_attention: bool = True
    ) -> torch.Tensor:
        """
        ...
        """
        h, A_base = self.forward_attention(
            h, attn_mask=attn_mask, attn_only=False
        )  # A == B x K x M
...
class ABMILGatedBaseConfig(PretrainedConfig):
    ...
    Attributes:
        ...
        moe_args (dict): Optional Mammoth MoE args for patch embedding.
