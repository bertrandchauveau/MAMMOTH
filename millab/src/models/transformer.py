...
class TransLayer(nn.Module):
    def __init__(
        self, norm_layer=nn.LayerNorm, dim=512, heads=8, dropout=0.1, mlp_dim=None
    ):
        super().__init__()
        self.norm = norm_layer(dim)
        self.attention = nn.MultiheadAttention(
            embed_dim=dim, num_heads=heads, dropout=dropout, batch_first=False
        )
    ...
    def forward(self, x, need_weights=False) -> torch.Tensor:
        ...
        norm_x = self.norm(x)
        out = self.attention(
            norm_x, norm_x, norm_x, need_weights=need_weights, average_attn_weights=True
        )
        if need_weights:
...
class Transformer(MIL):
    def __init__(
        ...
        make_trans_layer = lambda: TransLayer(
            dim=embed_dim, heads=num_heads, dropout=dropout, mlp_dim=encoder_mlp_dim
        )
