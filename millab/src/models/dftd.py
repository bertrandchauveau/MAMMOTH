...
class DFTD(MIL):
    def __init__(
        ...
        super(DFTD, self).__init__(
            in_dim=in_dim, embed_dim=embed_dim, num_classes=num_classes
        )
        self.classifier = Classifier_1fc(
            embed_dim, num_classes, droprate=dropout
        )  # .to(device)
        self.attention = GlobalGatedAttention(
            L=embed_dim, D=attention_dim, dropout=attention_drop
        )
        self.attCls = Attention_with_Classifier(
            L=embed_dim, D=attention_dim, K=1, num_cls=num_classes
        )
    ...
    def forward(
        ...
        results_dict = {
            "logits": logits,
            "loss": loss.to(h.dtype).to(h.device),
    ...
    def get_instance_loss(
        self, slide_sub_preds: torch.Tensor, slide_sub_labels: torch.Tensor
    ) -> torch.Tensor:
        """
        ...
        """
        instance_loss = self.inst_loss(
            slide_sub_preds, slide_sub_labels.squeeze(-1)
        ).mean()
        return instance_loss

    def forward_head(
        self, wsi_feats: torch.Tensor
    ...
    def get_cam_1d(
        self, classifier: torch.nn.Module, features: torch.Tensor
    ) -> torch.Tensor:
        """
    ...
    def forward_features(
        ...
        for tindex in index_chunk_list:
            ...
            patch_pred_logits = self.get_cam_1d(
                self.classifier, tattFeats.unsqueeze(0)
            ).squeeze(0)
        ...
        intermed_dict = {
            "slide_sub_preds": slide_sub_preds,
            "slide_sub_labels": slide_sub_labels,
        }
