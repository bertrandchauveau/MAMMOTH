...
class PPEG(nn.Module):
    def __init__(self, dim=512, k=7, conv_1d=False, bias=True):
        ...
        self.proj = (
            nn.Conv2d(dim, dim, k, 1, k // 2, groups=dim, bias=bias)
            if not conv_1d
            else nn.Conv2d(dim, dim, (k, 1), 1, (k // 2, 0), groups=dim, bias=bias)
        )
        self.proj1 = (
            nn.Conv2d(dim, dim, 5, 1, 5 // 2, groups=dim, bias=bias)
            if not conv_1d
            else nn.Conv2d(dim, dim, (5, 1), 1, (5 // 2, 0), groups=dim, bias=bias)
        )
        self.proj2 = (
            nn.Conv2d(dim, dim, 3, 1, 3 // 2, groups=dim, bias=bias)
            if not conv_1d
            else nn.Conv2d(dim, dim, (3, 1), 1, (3 // 2, 0), groups=dim, bias=bias)
        )
...
class DAttention(nn.Module):
    def __init__(
        self, input_dim=512, act="relu", gated=False, bias=False, dropout=False
    ):
...
class BClassifier(nn.Module):
    def __init__(
        self, input_size, output_class, dropout_v=0.0, nonlinear=True, passing_v=True
    ):  # K, L, N
...
class DSMIL(nn.Module):
    def __init__(
        ...
        self.i_classifier = nn.Sequential(nn.Linear(mlp_dim, num_classes))
    ...
    def attention(
        self, x, no_norm=False, label=None, criterion=None, return_attn=False
    ):
        ps = x.size(1)
        feats = x.squeeze(0)
        ...
        if return_attn:
            ...
            if self.attn_index == "max":
                ...
                if label is None:
                    pred = 0.5 * torch.softmax(
                        prediction_bag, dim=-1
                    ) + 0.5 * torch.softmax(classes_bag, dim=-1)
                    _, _attn_idx = torch.max(pred.squeeze(), 0)
                    attn = (
                        classes[:, int(_attn_idx)]
                        if self.cls_attn
                        else A[:, int(_attn_idx)]
                    )
                else:
                    attn = classes[:, label[0]] if self.cls_attn else A[:, label[0]]
            else:
                attn = (
                    classes[:, int(self.attn_index)]
                    if self.cls_attn
                    else A[:, int(self.attn_index)]
                )
    ...
    def forward(
        ...
        **kwargs,
        ...
        if mask_enable and (self.mask_ratio > 0.0 or mask_ids is not None):
        ...
        prediction_bag, attn, B, other = self.attention(
            x, no_norm, _label, _criterion, return_attn=return_attn
        )
...
def region_partition(x, region_size):
    ...
    regions = (
        x.permute(0, 1, 3, 2, 4, 5).contiguous().view(-1, region_size, region_size, C)
    )
    return regions
...
def region_reverse(regions, region_size, H, W):
    ...
    x = regions.view(
        B, H // region_size, W // region_size, region_size, region_size, -1
    )
...
class InnerAttention(nn.Module):
    ...
    def __init__(
        ...
        self.dim = dim
        self.region_size = (
            [region_size, region_size] if region_size is not None else None
        )  # Wh, Ww
        ...
        if region_size is not None:
            ...
            self.relative_position_bias_table = nn.Parameter(
                torch.zeros(
                    (2 * self.region_size[0] - 1) * (2 * self.region_size[1] - 1),
                    num_heads,
                )
            )  # 2*Wh-1 * 2*Ww-1, nH
...
class RegionAttntion(nn.Module):
        ...
        **kawrgs,
        ...
        if region_attn == "native":
            self.attn = InnerAttention(
                ...
                head_dim=head_dim,
                num_heads=num_heads,
                region_size=self.region_size if rpe else None,
                qkv_bias=qkv_bias,
                qk_scale=qk_scale,
                attn_drop=attn_drop,
                proj_drop=drop,
            ...
            self.attn = NystromAttention(
                dim=dim, dim_head=head_dim, heads=num_heads, dropout=drop
            )
    ...
    def padding(self, x):
        ...
        add_length = H * W - L
        if (
            add_length > L / (self.min_region_ratio + 1e-8) or L < self.min_region_num
        ) and not self.rpe:
            H, W = int(np.ceil(np.sqrt(L))), int(np.ceil(np.sqrt(L)))
            _n = -H % 2
            H, W = H + _n, W + _n
    ...
    def forward(self, x, return_attn=False):
        ...
        # partition regions
        x_regions = region_partition(
            x, region_size
        )  # nW*B, region_size, region_size, C

        x_regions = x_regions.view(
            -1, region_size * region_size, C
        )  # nW*B, region_size*region_size, C

        # R-MSA
        attn_regions = self.attn(
            x_regions, mask=self.attn_mask
        )  # nW*B, region_size*region_size, C
...
class RRTEncoder(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.pool = pool
        if self.pool == "attn":
            self.pool_fn = DAttention(
                self.final_dim, da_act, gated=da_gated, bias=da_bias, dropout=da_dropout
            )
        ...
        if pos == "ppeg":
            self.pos_embedding = PPEG(
                dim=mlp_dim, k=peg_k, bias=peg_bias, conv_1d=peg_1d
            )
        elif pos == "sincos":
...
class RRTMIL(MIL):
    def __init__(
        ...
        **kwargs,
        ...
        if moe_args is not None and moe_args.get("num_experts", 0) > 0:
            moe_args.update({"input_dim": in_dim, "dim": embed_dim})
    ...
    def forward(
        ...
        if return_moe_weights and isinstance(self.patch_embed, Mammoth):
            _, log_dict["moe_weights"] = self.patch_embed(h, return_weights=True)
...
class RRTMILModel(PreTrainedModel):
    ...
    def __init__(self, config: RRTMILConfig, **kwargs):
        ...
        self.model = RRTMIL(
            ...
            moe_args=getattr(config, "moe_args", {}),
        )
